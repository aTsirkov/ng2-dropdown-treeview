...
export class TreeviewItem {
    ...
    setCheckedRecursive(value: boolean, propagateSelectionDown: boolean) {
        if (_.isBoolean(value)) {
            if (!this.internalDisabled) {
                this.internalChecked = value;
                if (!_.isNil(this.internalChildren) && propagateSelectionDown) {
                    this.internalChildren.forEach(child => child.setCheckedRecursive(value, propagateSelectionDown));
                }
            }
        }
    }
    ...
    getCheckedItems(allowDataNonLeafe: boolean): TreeviewItem[] {
        ...
        if (_.isNil(this.internalChildren)) {
            ...
            if (this.internalChecked && allowDataNonLeafe) {
                checkedItems.push(this);
            }
            for (let i = 0; i < childCount; i++) {
                checkedItems = _.concat(checkedItems, this.internalChildren[i].getCheckedItems(allowDataNonLeafe));
            }
        }
