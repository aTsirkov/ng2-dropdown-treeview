...
<div class="treeview-container" [style.max-height.px]="maxHeight">
    <div *ngFor="let item of filterItems">
        <leo-treeview-item [item]="item" [template]="template || tpl" [config]="config" (checkedChange)="onItemCheckedChange(item, $event)">
        </leo-treeview-item>
    </div>
</div>
...
export class TreeviewComponent implements OnChanges {
    ...
    ngOnChanges(changes: SimpleChanges) {
        ...
        if (!_.isNil(itemsSimpleChange)) {
            ...
            this.raiseSelectedChange(this.config.allowDataNonLeafe);
        }
    }
    ...
    onAllCheckedChange(checked: boolean) {
        this.filterItems.forEach(item => {
            item.setCheckedRecursive(checked, true);
    ...
    raiseSelectedChange(allowDataNonLeafe: boolean) {
        this.checkedItems = this.getCheckedItems(allowDataNonLeafe);
        const values = this.eventParser.getSelectedChange(this);
        this.selectedChange.emit(values);
    }

    private getCheckedItems(allowDataNonLeafe: boolean): TreeviewItem[] {
        let checkedItems: TreeviewItem[] = [];
        if (!_.isNil(this.items)) {
            for (let i = 0; i < this.items.length; i++) {
                checkedItems = _.concat(checkedItems, this.items[i].getCheckedItems(allowDataNonLeafe));
            }
        }
